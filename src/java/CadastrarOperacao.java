...
public class CadastrarOperacao extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CadastrarOperacao</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CadastrarOperacao at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        float valor = Float.parseFloat(request.getParameter("operacao") + request.getParameter("valor"));
...
"            </ul>");
        ...
        try {
            ...
            PreparedStatement stm = conexao.prepareStatement("INSERT INTO `operacoes`(`numConta`, `valor`, `data`,`cpfResponsavel`) VALUES (?, ?, ?, ?)");
            stm.setString(1, numero);
            stm.setFloat(2, valor);
            stm.setString(3, dateFormat.format(data));
            stm.setString(4, cpfResponsavel);
            stm.execute();
            conexao.close();
